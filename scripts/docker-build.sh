...
main() {
  ...
  if [ ! -z ${http_proxy} ]; then
  	...
  	cat ../${PROJET}/Dockerfile | sed "s#http_proxy_disabled#http_proxy=${http_proxy}#g" > ../${PROJECT}/Dockerfile.proxy 
  	cat ../${PROJET}/Dockerfile | sed "s#https_proxy_disabled#https_proxy=${https_proxy}#g" >> ../${PROJECT}/Dockerfile.proxy
  	cat ../${PROJET}/Dockerfile | sed "s#no_proxy_disabled#no_proxy=\"${DOWNLOAD_HOST},docker,${no_proxy}\"#g" >> ../${PROJECT}/Dockerfile.proxy
  	cat ../${PROJET}/Dockerfile | sed "s#DOWNLOAD_BASE_URL=\"\([^\"]*\)\"#DOWNLOAD_BASE_URL=\"${DOWNLOAD_BASE_URL}\"#g" >> ../${PROJECT}/Dockerfile.proxy
